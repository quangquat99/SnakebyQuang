...
void Draw(const int &dong,const int &cot,const int &kitu,const int &mau, COORD &pos, ColorCharacter &cc)
{
    pos.X = cot;
    pos.Y = dong;
    cc.character = kitu;
...
void DrawString(const string &str,const int &dong,const int &cot,const int &mau)
{
    int icot = cot;
    for( int i = 0 ; i < str.length(); i ++)
    {
        Draw(dong, icot, str[i], mau, pos, cc);
        icot++;
    }
}
...
void DiemCao(Snake &snake)
{
    ifstream instream;
    instream.open("Diemcao.txt", ios::in);
    int diemcao;
    instream>> diemcao;
    snake.highScore=diemcao;
    string diemcaoSTR = genericToString(diemcao);
    DrawString("Diem cao : " + diemcaoSTR, 20, 35, 7);
    instream.close();
}

void DrawScreen( Snake &snake)
{
...
bool KiemtraItem(const Snake &snake,const HoaQua &hq,const int &chisoItem,const vector<Items> &listItems)
{
    bool check = true;
...
void HienThi( Snake &snake,const HoaQua &hq,const NongDan &nd,const Items &item,const vector<Items> &listItems,const int &chisoItem)
{
    // in ra rao
    DrawScreen(snake);
...
void ChoiGame()
{
    ...
    while (1)
    {
       ...
       if ( check == -1 || check == -2 )
       {
        ...
        ofstream outstream ("Diemcao.txt");
        outstream << snake.highScore;
        outstream.close();
        }
